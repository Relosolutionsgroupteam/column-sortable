...
class SortableLink
{
    ...
    private static function determineDirection($sortColumn, $sortParameter)
    {
        ...
        if (request()->get('orderBy') == $sortParameter && in_array(request()->get('sortedBy'), ['asc', 'desc'])) {
            $icon      .= (request()->get('sortedBy') === 'asc' ? config('columnsortable.asc_suffix', '-asc') :
    ...
    private static function getAnchorClass($sortColumn, &$anchorAttributes = [])
    {
        ...
        if ($directionClassPrefix !== null && self::shouldShowActive($sortColumn)) {
            $class[] = $directionClassPrefix.(request()->get('sortedBy') === 'asc' ? config('columnsortable.asc_suffix', '-asc') :
                    config('columnsortable.desc_suffix', '-desc'));
        }
